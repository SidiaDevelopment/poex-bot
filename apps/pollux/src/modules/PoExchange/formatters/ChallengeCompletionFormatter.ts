...
export class ChallengeCompletionFormatter implements IPoExchangeFormatter {
    public format(embed: EmbedBuilder, user: IPoExchangeUser, services: IPoExchangeService[], links: IPoExchangeLinks): void {
        ...
        lines.push(`\`\`\`@${user.name} ${translate("poex.format.whispers.challengeCompletion")}\`\`\``)
        lines.push(formatLinks(links))

        embed.setTitle(translate("poex.format.titles.challengeCompletion"))
