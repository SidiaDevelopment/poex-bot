...
export class BossCarryFormatter implements IPoExchangeFormatter {
    public format(embed: EmbedBuilder, user: IPoExchangeUser, services: IPoExchangeService[], links: IPoExchangeLinks): void {
        ...
        lines.push("")
        lines.push(`\`\`\`@${user.name} ${translate("poex.format.whispers.bossKilling")}\`\`\``)

        lines.push(formatLinks(links))

        embed.setTitle(translate("poex.format.titles.bossKilling"))
